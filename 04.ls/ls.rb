...
def main
  ...
  if options[:long_format]
    puts "total #{total_blocks(path, options[:show_all])}"
    files.each do |file|
      file_path = "#{path}/#{file}"
...
def show_file_info(path)
  ...
  hard_link_number = "%2d" % file_stat.nlink
  owner_name = Etc.getpwuid(file_stat.uid).name
  group_name = "%3s" % Etc.getgrgid(file_stat.gid).name
  file_size = "%6d" % file_stat.size
...
def permissions_to_s(mode)
  ...
  perms = [String.new('---'), String.new('---'), String.new('---')]
